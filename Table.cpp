...
void Table::show() const
{
	...
	// 绘制表头
	wchar_t header[4][3] = { L"ID", L"姓名",L"等级",L"积分" };	// 表头数据
	...
	for (int j = 40; j < 370; j += 30)
	{
		if (i < manager.size())	// 在不超出索引范围的情况下才读取数据
		{
			...
			_itow_s(manager.getPerson(i).id, buffer, 10);
			drawtext(buffer, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

			rect = { 130, j, 130 + 120, j + 30 };
			drawtext(manager.getPerson(i).name, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

			rect = { 250, j, 250 + 120, j + 30 };
			drawtext(manager.getPerson(i).Vip, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

			rect = { 370, j, 370 + 120, j + 30 };
			_itow_s(manager.getPerson(i).score, buffer, 10);
			drawtext(buffer, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
...
void Table::showTable(int index)
{
	...
	// 绘制表头
	wchar_t header[4][3] = { L"ID", L"姓名", L"等级",L"总分"};	// 表头数据
	for (int i = 10, j = 0; i < 490 && j < 4; i += 120, j++)
	{
		rect = { i, 10, i + 120, 10 + 30 };
		drawtext(header[j], &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
	...
	_itow_s(manager.getPerson(index).id, buffer, 10);
	drawtext(buffer, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

	rect = { 130, 40, 130 + 120, 70 };
	drawtext(manager.getPerson(index).name, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

	rect = { 250, 40, 250 + 120, 70 };
	drawtext(manager.getPerson(index).Vip, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

	rect = { 370, 40, 370 + 120, 70 };
	_itow_s(manager.getPerson(index).score, buffer, 10);
	drawtext(buffer, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
}
